...
function displayRecipeInfo(recipe) {
  ...
  var queryURL =
    'https://api.yummly.com/v1/api/recipes?_app_id=309f5cb2&_app_key=' +
    '6f553c1834e2e101ee5e1fb2d12d7ff6&q=' +
    recipe +
    '&requirePictures=true&maxResult=8&start=0';
  ...
  $.ajax({
    ...
    response.matches.forEach(function(item) {
      ...
      var createFav = $('<a>').addClass(
        'waves-effect waves-light btn deep-orange fav-recipe'
      );
      ...
      var ingredients = item.ingredients.map(
        ingredient => '  ' + `${ingredient}`
      );

      var pThree = $('<p>').html(
        "<span class='heading6'>Ingredients:</span> " + ingredients
      );
      ...
      var url =
        ...
        recipeID +
        '?_app_id=309f5cb2&_app_key=6f553c1834e2e101ee5e1fb2d12d7ff6';
